//! `rivet_py`: the simulator-independent parts of Rivet's kit for Python,
//! so a cocotb testbench can migrate piece by piece.
//!
//! ```python
//! import rivet_py as rv
//! mem = rv.Memory()
//! mem.load_hex("prog.hex", base=0, word_bytes=4)
//! rng = rv.Rng(seed=1)
//! sb = rv.Scoreboard("fifo")
//! cg = rv.Covergroup("axi"); pt = cg.point("len", rv.Bins().bin("short", 1, 4).bin("long", 5, 16))
//! ```
...
#[pyclass(name = "Scoreboard", unsendable)]
struct Scoreboard {
...
#[pyclass(name = "CoverPoint", unsendable)]
struct CoverPoint {
...
#[pyclass(name = "Covergroup", unsendable)]
struct Covergroup {
...
#[pymodule]
fn rivet_py(m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Memory>()?;
    m.add_class::<Rng>()?;
